"""
Standalone GridOps worker — consumes cyrus.gridops queue.
"""
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [gridops_worker] %(levelname)s %(message)s",
)
log = logging.getLogger(__name__)


class GridOpsWorker(BaseConsumer):
    queue = settings.QUEUE_GRIDOPS

    def __init__(self) -> None:
        super().__init__()
        self._agent = GridOpsAgent()

    def handle_message(self, channel: pika.channel.Channel, payload: dict[str, Any]) -> None:
        job_id = payload.get("job_id", "unknown")
        log.info("[gridops_worker] Processing threat for job %s", job_id)
        ...
        redis_client.store_agent_report(job_id, "gridops", report.model_dump())
        ...
        log.info(
            "[gridops_worker] Job %s complete — %d actions, status=%s",
